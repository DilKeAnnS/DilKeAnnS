...
function displayExamUpdates(updates) {
    ...
    let html = '';
    const grouped = {};
    updates.forEach(update => { if (!grouped[update.source]) grouped[update.source] = []; grouped[update.source].push(update); });
    for (const [source, sourceUpdates] of Object.entries(grouped)) {
        html += `<div class="exam-source-card"><div class="exam-source-header"><span class="source-icon">📢</span><span class="source-name">${source}</span><span class="update-count">${sourceUpdates.length} updates</span></div><div class="exam-list">`;
        sourceUpdates.forEach(update => {
            const formattedDate = new Date(update.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
            html += `<div class="exam-item"><div class="exam-title"><a href="${update.link}" target="_blank" rel="noopener noreferrer">📌 ${update.title}</a></div><div class="exam-meta"><span class="exam-date">📅 ${formattedDate}</span><a href="${update.link}" target="_blank" class="exam-link">🔗 Official Notice →</a></div></div>`;
        });
        html += `</div></div>`;
    }
...
if (typeof window !== 'undefined') {
    fetchAllExamUpdates();
    setInterval(fetchAllExamUpdates, 60 * 60 * 1000);
}
